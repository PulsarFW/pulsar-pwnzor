...
CreateThread(function()
	...
	while true do
		for k, v in ipairs(GetPlayers()) do
			...
			if mult > 1.0 then
				local player = plsr.Fetch:Source(tonumber(v))
...
AddEventHandler("playerDropped", function()
	plsr.Pwnzor.Players:Disconnected(source)
end)
...
AddEventHandler("explosionEvent", function(sender, ev)
	...
	local player = plsr.Fetch:Source(src)
	for _, v in ipairs(Config.Components.Explosions.Options.Types) do
		if ev.explosionType == v then
			...
			then
				...
				if char then
					plsr.Logger:Warn("Pwnzor", string.format("%s %s (%s) Triggered Explosion Detection With Explosion Type of %s", char:GetData("First"), char:GetData("Last"), char:GetData("SID"), ev.explosionType), {
						console = true,
						file = false,
						database = true,
						discord = {
					...
					plsr.Pwnzor:Screenshot(char:GetData("SID"), "Potential Weapon Exploit")
				else
					plsr.Logger:Warn("Pwnzor", string.format("Source %s Triggered Explosion Detection With Explosion Type of %s", src, ev.explosionType), {
						console = true,
						file = false,
						database = true,
						discord = {
