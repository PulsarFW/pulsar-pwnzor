...
CreateThread(function()
	Wait(30000)
	while true do
		if plsr.State.flags.loggedIn and not plsr.State.flags.inCCTVCam and not plsr.State.flags.inHeliCam then
			...
			if (IsPedArmed(ped, 1) or IsPedArmed(ped, 2) or IsPedArmed(ped, 4)) and not WhitelistedWep[weapon] then
				...
				if not inVeh and (CamPos == 0 or CamPos == 1 or CamPos == 2) and CamFov > 60.2 then
					plsr.Callbacks:ServerCallback("Pwnzor:FOV", { fov = CamFov })
				...
				elseif inVeh then
					if CamPosVeh == 4 and CamFovCar > 60.6 then
						plsr.Callbacks:ServerCallback("Pwnzor:FOV", { fov = CamFovCar })
